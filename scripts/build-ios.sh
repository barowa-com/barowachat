...
# export BAROWACHAT_INSTALL_IPA=1

### Rotate IDs ###
[ -n "${BAROWACHAT_NEW_GROUP}" ] && {
	# App group IDs
	sed -i "" "s/group.${BAROWACHAT_ORIG_GROUP}.app/group.${BAROWACHAT_NEW_GROUP}.app/g" "ios/BarowaChat Share/BarowaChat Share.entitlements"
	sed -i "" "s/group.${BAROWACHAT_ORIG_GROUP}.app/group.${BAROWACHAT_NEW_GROUP}.app/g" "ios/Runner/Runner.entitlements"
	sed -i "" "s/group.${BAROWACHAT_ORIG_GROUP}.app/group.${BAROWACHAT_NEW_GROUP}.app/g" "ios/Runner.xcodeproj/project.pbxproj"
	# Bundle identifiers
	sed -i "" "s/${BAROWACHAT_ORIG_GROUP}.app/${BAROWACHAT_NEW_GROUP}.app/g" "ios/Runner.xcodeproj/project.pbxproj"
}

[ -n "${BAROWACHAT_NEW_TEAM}" ] && {
	# Code signing team
	sed -i "" "s/${BAROWACHAT_ORIG_TEAM}/${BAROWACHAT_NEW_TEAM}/g" "ios/Runner.xcodeproj/project.pbxproj"
}
cat << EOHELP
...
2. Fix it yourself
    ...
    - Ensure that Xcode created the App ID successfully (for barowachat.app and barowachat.app.BarowaChat-Share)
...
[ -n "${BAROWACHAT_INSTALL_IPA}" ] && {
  ...
  cfgutil --foreach install-app "${TMPDIR}/barowachat.ipa"
  rm -rf "${TMPDIR}"
}
